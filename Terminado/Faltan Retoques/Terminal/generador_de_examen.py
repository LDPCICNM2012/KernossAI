...
def proceso_ia_responder(texto): #Funcion que hace que la ia procese los datos que se han puesto por el usuario con x instrucciones. Basicamente lo que engloba todo lo que se encarga la ia
    ...
    while True:
        ...
        print("\n[IA]: ", end="")
        stream_resp = ollama.chat(model="llama3.2", messages=conversacionales, stream=True)
        feedback = ""
...
def proceso_ia_corregir(examen_memoria):
    ...
    conversacionales = [
        ...
        {"role": "user", "content": f"Este es el examen. NO lo corrijas tú. NO des las respuestas correctas. Solo acusa recibo y espera a que yo te dé MIS respuestas:\n\n{contenido_examen}"}
    ]

    # Llamada inicial para que la IA acuse recibo del examen y pida las respuestas
    print("\n[IA]: ", end="")
    stream_inicial = ollama.chat(model="llama3.2", messages=conversacionales, stream=True)
    respuesta_inicial = ""
    ...
    while True:
        ...
        if respuesta.upper() == "FIN":
            ...
            if exportar_correccion.upper() == "S":
                ...
                # Usamos el AppleScript para guardar
                try:
                    ruta_guardar = subprocess.check_output(['osascript', '-e', ascript]).decode('utf-8').strip()
                    if ruta_guardar:
        ...
        print("\n[IA Corrigiendo]: ", end="")
        stream = ollama.chat(model="llama3.2", messages=conversacionales, stream=True)
